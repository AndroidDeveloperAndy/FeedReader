...
import com.hackspace.andy.readrss.utils.Pixel;
...
public class PullToRefreshComponent {
	...
	private static final String TAG = PullToRefreshComponent.class.getName();
	...
	private void initialize() {
		...
		this.listView.setOnTouchListener((v, event) -> {
	...
	private void beginRefresh(View viewToUpdate,
		...
		new Thread(() -> {
            try {
                ...
                PullToRefreshComponent.this.runOnUiThread(() -> PullToRefreshComponent.this
	...
	private boolean isIncremental(int from, int to, int step) {
		...
		Log.i(TAG, "scrolling from " + String.valueOf(realFrom));
		Log.i(TAG, "scrolling to " + String.valueOf(realTo));
